...
[[nodiscard]] std::unique_ptr<Game> Terminate::Calculation(Data& data) {
	std::this_thread::sleep_for(5s);
	data.state = false;
	return nullptr;
}

void Terminate::Draw(Data data)const {
	scary_font(U"terminate").draw();
}
...
[[nodiscard]] std::unique_ptr<Game>	Menu::Calculation(Data& data) {
	if (!KeySpace.down())return nullptr;
	else return std::make_unique<Terminate>(U"fafa");
}
